...
interface ShapeBuilderFormProps {
...
export function ShapeBuilderForm({ product, onConfigChange }: ShapeBuilderFormProps) {
  ...
  useEffect(() => {
    console.log('[ShapeBuilderForm] Emitting config change:', config);
  ...
  // Get the current shape type to conditionally show/hide fields
  const currentShapeType = (config['shapeType'] as string) || 'rectangle';
  ...
  const renderParameter = (param: ProductParameter) => {
    ...
    // Skip showing height/width fields depending on shape type
    if (param.id === 'height' && (currentShapeType === 'circle' || currentShapeType === 'hexagon')) {
      return null;
    }
    
    if (param.id === 'width' && (currentShapeType === 'circle' || currentShapeType === 'hexagon')) {
      return null;
    }
    
    if (param.id === 'diameter' && (currentShapeType === 'rectangle')) {
      return null;
    }
