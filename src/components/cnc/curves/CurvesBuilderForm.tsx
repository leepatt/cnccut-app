...
export function CurvesBuilderForm({ product, onConfigChange, derivedMeasurements }: CurvesBuilderFormProps) {
  ...
  useEffect(() => {
    console.log('[CurvesBuilderForm] Emitting config change:', config);
  ...
  // Render derived measurements section
  const renderDerivedMeasurements = () => {
    if (!derivedMeasurements) return null;
    
    return (
      <div className="mt-6">
        <Separator className="my-3" />
        <h3 className="text-sm font-medium mb-3">Calculated Measurements</h3>
        <div className="space-y-3">
          <div>
            <Label className="block mb-1 text-muted-foreground">Arc Length (L)</Label>
            <div className="flex items-center">
              <span className="text-lg font-medium">{derivedMeasurements.arcLength.toFixed(1)}</span>
              <span className="ml-1 text-sm text-muted-foreground">mm</span>
            </div>
          </div>
          <div>
            <Label className="block mb-1 text-muted-foreground">Chord Length (c)</Label>
            <div className="flex items-center">
              <span className="text-lg font-medium">{derivedMeasurements.chordLength.toFixed(1)}</span>
              <span className="ml-1 text-sm text-muted-foreground">mm</span>
            </div>
          </div>
        </div>
